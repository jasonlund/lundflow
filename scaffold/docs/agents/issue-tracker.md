...
this file only translates it into the operations the engineering skills name.

## Conventions

- **Create an issue**: `save_issue(team: "<team>", title: "…", description: "…")`
  with no `id`. Markdown body, literal newlines — never escape sequences.
- **Read an issue**: `get_issue(id: "{PREFIX}-123")`; add `list_comments` when the
  discussion matters.
- **List issues**: `list_issues(team: "<team>", state: "Todo", label: "…",
  fields: ["id","title","status","labels","assignee"])`.
- **Update an issue**: `save_issue(id: "{PREFIX}-123", …)`. Prefer the `patch` ops
  for surgical body edits over resending a whole description.
...
- **Close**: `save_issue(id: "{PREFIX}-123", state: "Done")`. Lifecycle is
...
- **Branch ↔ ticket**: a branch name carries every ticket id with the owner prefix
  dropped, ≤40 chars (`{prefix}-277-adopt-new-skills`, the ids lowercased).
  Resolve the ticket by upper-casing the leading `{prefix}-NNN`.
...
a bare `#42` is always a GitHub PR, `{PREFIX}-42` always a Linear issue.

## When a skill says "publish to the issue tracker"

Create a Linear issue on team `<team>` with `save_issue`.

## When a skill says "fetch the relevant ticket"

`get_issue(id: "{PREFIX}-123")`. Given only a branch or PR, extract the id per
**Branch ↔ ticket** above.
