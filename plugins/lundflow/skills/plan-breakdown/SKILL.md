...
an integration split into 5 sub-tickets that all edit one API service class on a
hard dependency chain is forced serial, zero parallelism gained). This skill turns
a PRD into tickets that are **TDD-able, DDD-clean, and actually parallelizable**,
then hands each ticket to `lundflow:plan-slices` for slice planning.
...
confirms. `lundflow:plan-slices` (the back half) stays side-effect-free.
...
- **The demoable-alone test.** Can this ticket be demonstrated or verified on its
  ...
  heartbeats folded into each leg's own ticket pass it.
...
chokepoints, waves, DDD boundaries — are this kit's own.
...
- **Collaborator extraction is a last resort, not a reflex.** Recommend splitting a
  ...
  shape (e.g. a sibling integration's single API service class). Cohesion + serial
  often beats fragmentation + parallel.
...
- **Per sub-ticket, produce:**
  ...
  - **Acceptance criteria** — a checkbox list of what "done" means, each
    ...
    a prose scope paragraph gives `/lundflow:review:debrief`'s spec pass nothing to verify.
  ...
  - a branch name (Linear convention: drop the `<user>/` prefix Linear prepends,
    ≤40 chars)
- **State the concurrency ceiling explicitly.** Spell out the realistic shape, e.g.
  *"foundation ticket → 3-wide fan-out → 2 dependents; tickets X and Y share file Z
  so they serialize / cost one reconcile."* This is the analysis the serial split
  above lacked.
- **Build the concurrency graph artifact.** `blocks`/`blocked-by` relations are
  ...
  - a **Mermaid graph** (Linear renders ```mermaid blocks) showing the same
    ...
    visually obvious. Example shape, where `{PREFIX}` is the *Ticket prefix*
    setting:
    ````
    ```mermaid
    graph LR
      subgraph Wave1
        T1[{PREFIX}-201 order-import]
      end
      subgraph Wave2
        T2[{PREFIX}-202 invoice-mutations]
      end
      T1 --> T2
    ```
    ````
...
Use the `linear-server` MCP (`save_issue`). Per the Linear convention (*Linear* in
...
- Write each ticket's **plan** (What to build + acceptance criteria + decisions +
  ...
  the self-contained body; `lundflow:plan-slices` appends the other half next.
...
For each created ticket, **in dependency order**, invoke the `lundflow:plan-slices` flow
(surface classify → observable behaviors → testability gate → 2–6-test slices →
honest-RED notes → traceability). `lundflow:plan-slices` **appends the slice backlog into that
same ticket body**, so one ticket = one self-contained body an executor can pick up
and run `lundflow:tdd` against. Then **stop** — execution is the `lundflow:tdd` skill's job, unchanged.

Phase D creates each sub-ticket in the default (Backlog) status; it sets no status
itself. Each sub-ticket reaches **Todo** through its own `lundflow:plan-slices` pass here
(the *Automatic ticket status transitions* contract in
`.ai/guidelines/lundflow-linear.md`), not from breakdown. The parent ticket's status
is left untouched.

## Reference

- `${CLAUDE_PLUGIN_ROOT}/skills/plan-slices/SKILL.md` — the back half; slice planning + the
