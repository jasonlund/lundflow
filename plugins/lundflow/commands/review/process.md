...
4. When any fix touched frontend code, run the render check per *Browser verification*
   in `.ai/guidelines/lundflow-workflow.md`. An unverifiable check does not stop the run.
...
- Browser verification: {verified | ⚠️ Not browser-verified — {reason} | n/a (no frontend change)}
```
